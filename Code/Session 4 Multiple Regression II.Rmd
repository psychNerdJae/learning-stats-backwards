...
Without knowing it, we've already performed multiple regression with categorical variables, in the sense that a single categorical variable must be decomposed into multiple dummy predictors. Now, we'll expand our toolbox so that we can interpret models with multiple categorical variables.

It is natural to expect that different penguin species will have different average body masses. Another reasonable expectation is that there will be sex differences in body mass.

Let's first plot out those data to get a sense for what our analysis will be doing:
...
We can examine both in the same model using the same notation we've been using:
...
In the dataset, there are also observations about what island each penguin lived on: Biscoe, Dream, and Torgersen. Your biologist friend has a hypothesis that some islands have richer food sources for the penguins

```{r}
penguins %>%
  lm(body_mass_g ~ species + island,
     data = .) %>%
  tidy()
```

We can use regression to test the questions we actually care about, by making smart use of contrasts.
