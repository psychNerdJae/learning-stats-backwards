...
Let's try it and see what result we get.
...
On its own, we saw that bill length significantly predicted body mass. But in a regression that also includes flipper length, we see that it's no longer significant.

Let's see if we can plot these data to get an intuitive sense for these results. As our regression results showed, there's a very strong relationship between flipper length and body mass. If there were an *additional* relationship between bill length and body mass (above-and-beyond an effect of flipper length), what would we expect to see? Keeping flipper length constant, we'd expect to see that longer bills are associated with greater body mass. Is there evidence of this? Try taking your mouse cursor (or finger), and tracing the vertical line where flipper length is 200mm. As we move from the bottom of the plot to the top, we'd expect to see purple dots first, then pink, then orange. This is not the pattern we observe, and so we cannot reject the nihilist's null hypothesis.
...
Now let's move on to the estimate for bill length. What does this number mean? Try explaining it out loud before reading further... This estimate says that for every $1$mm increase in bill length, a penguin's body mass increases by $75.28$g on average, *adjusting for bill depth*. You might also hear this expressed as *controlling for bill depth*, or something like *holding bill depth constant*. What does that mean? Let's say that you've got two penguins, Archie and Betty. Both of them have the exact same bill depth, but Archie's bill is 5mm longer. Can you predict how much heavier Archie is than Betty? This is a very easy thing to compute, because we're making a prediction from bill length, holding bill depth constant. Of course, in the real world, it's very doubtful that we'd be able to find a colony of penguins that all have the exact same bill depth. Despite this, we want to be able to say something about bill length. Regression does this by finding ***semi-partial*** estimates of the relationship strength between each predictor and your outcome variable. This is a fancy way of saying that the beta estimates reflect each predictor's *unique* contribution to the overall variance explained in the outcome variable. So, no matter what a penguin's bill depth is, we know that a $1$mm increase in bill length is associated with a $75.28$g increase in body mass.
